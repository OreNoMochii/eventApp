...
export default class NavBar extends React.Component<{}, { 
  ...
  componentDidMount() {
    ...
    fetch( `/user/${this.state.currentUser}` )
      .then(response => response.json())
        .then((response) => {
            if ( this.state.isMounted ) {
                this.setState( { currentUser: localStorage.getItem('user_name') } );
            }
        } ).catch( err => {
          console.log(err);
    });
        ...
        <Navbar className= "navbar navbar-expand-lg bg-dark fixed-top" dark id="mainNav">
          ...
          <Collapse  isOpen={!this.state.collapsed} id="navbarResponsive" navbar>
            ...
            <NavItem className="navbar-brand text-white">
               { (this.state.currentUser !== null || undefined) ?  `Welcome, ${localStorage.getItem('user_name')} Click on an event to unsign for it` : `Loading..` }
              </NavItem>
              ...
              <NavItem className="navbar-brand">
              <Button className=" text-white background=white" href="/events"> list of events </Button>
              </NavItem>
